...
export default function getRoutes(mountpath) {
    ...
    const routes = [
        { path: `${root}/`, action: (store) => <ListPostsPage store={store} />},
        { path: `${root}/list`, action: (store) => <ListPostsPage  store={store}/> },
        { path: `${root}/post/`, action: (store) => <ViewPostPage store={store}/> }
    ];
    return routes;
}